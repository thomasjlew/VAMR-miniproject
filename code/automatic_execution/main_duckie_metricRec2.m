...
filename_GIF_traj = 'traj_gif_duckie_MC_frame_3_2';
latest_adj_3d_pts = [];

% Bundle Adjustment (BA) parameters
% Metric reconstruction by marker detection and scaling
doMetricReconstruction = true; % only implemented for Duckie dataset
...
elseif ds == 3
    img0 = rgb2gray(imread([duckie_path ...
        sprintf('/frame3/%04d.jpg',bootstrap_frames(1))]));
%         sprintf('/drive 1hz full/%04d.jpg',bootstrap_frames(1))]));
    img1 = rgb2gray(imread([duckie_path ...
        sprintf('/frame3/%04d.jpg',bootstrap_frames(2))]));
%         sprintf('/drive 1hz full/%04d.jpg',bootstrap_frames(2))]));
...
for frameCount = range
    ...
    elseif ds == 3 %Duckie
        image = im2uint8(rgb2gray(imread([duckie_path ...
            sprintf('/frame3/%04d.jpg',frameCount)])));
...
%     try
        ...
        %% Plot trajectory
        if live_plotting 
            plotTrajectory_small(f_cameraTrajectory,f_trackingP,camOrientations,camLocations,...
...
% %            rethrow(e)
%         fprintf('Error in frame nb '); fprintf('%d',frameCount);
%         
%         keypointCount = cat(1,keypointCount,0);
%         camOrientations = cat(3,camOrientations,camOrientations(:,:,end));
%         camLocations = cat(1,camLocations,pose(:,4)');
%         scores = cat(1,scores,0);
%     end 
