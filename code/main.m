...
if ds == 0
    ...
    'MaxNumTrialsRansac',2000, ...            
    'ConfidenceRansac',70,...
    'MaxReprojectionErrorRansac', 3, ...
...
elseif ds == 2
    ...
    'ConfidencePnP',95,...
    'MaxReprojectionErrorPnP', 3, ...
    'MaxNumTrialsRansac',3000, ...            
    'ConfidenceRansac',50,...
    'MaxReprojectionErrorRansac', 6, ...
    ... % Triangulation parameters
    'AlphaThreshold', 4 *pi/180, ...   %Min baseline angle [rad] for new landmark (alpha(c) in pdf)
    ... % Harris paramters for canditate keypoint exraction
...
camRotations= R_initial;
camLocations = t_initial;
...
for i = range
    ...
    camRotations = cat(3,camRotations,pose(:,1:3));
    ...
    figure(f_cameraTrajectory);
        ...
        cam.Orientation = camRotations(:,:,end);
        % plot landmarks
